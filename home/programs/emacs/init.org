  ...
  (pretty-hydra-define main-hydra
    ...
    ("File"
     ...
     (("a" org-agenda "Agenda")
      ...
      ("c" my/org-capture-fleeting "Capture Note")
      ("C" nil "View Fleeting"))
     "Tool"
     (("z" vundo "Undo Tree")
      ("g" magit-status "Magit")
      ("t" vterm "Terminal")
      ("SPC" major-mode-hydra "Major Hydra"))))
  (setup-expecting "avy"
...
(setup "key-chord"
  (key-chord-mode 1)
  (key-chord-define-global "jk" 'main-menu))
#+end_src

*** meow
1年ぐらいつかった
もっと薄いモードレスなパッケージの方がいいかも
at-point, on-bufferな風味がある機能はEmbarkにやらせて、それ以外の部分をtransient(とrepeat-mode, devil.elとか)にやらせるか
#+begin_src emacs-lisp :tangle no
(setup "meow"
  (!when (equal system-type 'darwin)
    (defun my/mac-deactivate-input-method ()
      (progn
        (when (and (equal window-system 'mac) (fboundp 'mac-select-input-source))
          (mac-select-input-source "com.apple.inputmethod.Kotoeri.RomajiTyping.Roman"))
        (when (and (equal window-system 'ns) (fboundp 'mac-ime-deactivate))
          (mac-ime-deactivate))))
    (defun my/mac-deactivate-ime-without-insert ()
      (when (and (frame-focus-state) (not (meow-insert-mode-p)))
        (my/mac-deactivate-input-method)))
    (setup-hook 'meow-insert-exit-hook 'my/mac-deactivate-input-method)
    (add-function :after after-focus-change-function #'my/mac-deactivate-ime-without-insert))
...
(defun meow-setup ()
  ...
  (meow-motion-overwrite-define-key
   ...
   '("; g" . magit)
   '("; t" . ("Term" . vterm))
   '("; n" . ("Find Roam" . org-roam-node-find))
   '("<escape>" . ignore))
  ...
  (meow-normal-define-key
   ...
   ;; selection
   '("v l" . ("line" . meow-line))
   '("v i" . ("Inner things" . meow-inner-of-thing))
   '("v a" . ("Bound things" . meow-bounds-of-thing))
   '("v b" . ("Block" . meow-block))
   '("v c" . ("Inner" . puni-mark-list-around-point))
   '("v x" . ("Bound" . puni-mark-sexp-around-point))
   '("v s" . ("Symbol" . meow-mark-symbol))
   '("v w" . ("Word" . meow-mark-word))
   '("v v" . ("ExpReg" . puni-expand-region))
   '("v r" . ("Rectangle" . rectangle-mark-mode))
   ...
   '(", <" . ("<a>" . puni-wrap-angle))
   '(", d" . ("del" . puni-splice))
   '(", l" . ("slurp" . puni-slurp-forward))
   '(", h" . ("barf" . puni-barf-forward))
   ...
   '("; t" . ("Term" . vterm))
   '("; n" . ("Find Roam" . org-roam-node-find))
   '("; j" . ("Journal" . org-roam-dailies-capture-today))
   '("; c" . ("Capture" . my/org-capture-fleeting))
   '("; l" . ("LSP" . lsp-menu))
